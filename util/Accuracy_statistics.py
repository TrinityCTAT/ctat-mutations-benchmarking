...
def plot_f1_score(input_table_filenames, output_dir):
    ...
    for filename in input_table_filenames:
        ...
        if typename in seen:
            if bname in seen:
                raise RuntimeError(
                    "Error, converted {} to {} for unique type but not unique".format(
                        filename, typename
                    )
                )
            else:
    ...
    df_accuracy_stats["F1"] = (
        2 * df_accuracy_stats["sn"] * df_accuracy_stats["ppv"]
    ) / (df_accuracy_stats["sn"] + df_accuracy_stats["ppv"])

    ## Compute accuracy
    df_accuracy_stats["Accuracy"] = (df_accuracy_stats["tp"]) / (
        df_accuracy_stats["tp"] + df_accuracy_stats["fp"] + df_accuracy_stats["fn"]
    )

    ## Mathews correlation coefficient : (TP*TP - FP*FN)/ (TP+FP)*(TP+FN)
    df_accuracy_stats["MCC"] = (
        (df_accuracy_stats["tp"] * df_accuracy_stats["tp"])
        - (df_accuracy_stats["fp"] * df_accuracy_stats["fn"])
    ) / (
        (df_accuracy_stats["tp"] + df_accuracy_stats["fp"])
        * (df_accuracy_stats["tp"] + df_accuracy_stats["fn"])
    )
    ...
    f1 = df_accuracy_stats[df_accuracy_stats["eval_min_rna_cov"] == 10].sort_values(
        by=["F1"]
    )[::-1]
    f1["F1"] = f1["F1"] * 100
    ...
    plt.figure()
    ranks = (
        df_accuracy_stats.groupby("Type")["Accuracy"].max().sort_values()[::-1].index
    )
    acc = df_accuracy_stats[df_accuracy_stats["eval_min_rna_cov"] == 10].sort_values(
        by=["Accuracy"]
    )[::-1]
    ...
    sns_plot = sns.boxplot(
        y="MCC", x="Type", order=ranks, data=df_accuracy_stats, palette="tab10"
    )
    sns_plot.set_xticklabels(sns_plot.get_xticklabels(), rotation=90)
    fig = sns_plot.get_figure()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawTextHelpFormatter, description="Plots F1 score \n"
    )
    parser.add_argument(
        "--input_accuracy_table", nargs="+", required=True, help="List of input files"
    )
