...
sys.path.insert(
    0, os.path.sep.join([os.path.dirname(os.path.realpath(__file__)), "pylib_common"])
)
...
def audit_command_info(output_dir, argv):
    ...
    cmd_audit_filename = os.path.join(
        output_dir, "__benchmark_cmd.{}.txt".format(time.time())
    )
...
def main():
    ...
    required_opts_group.add_argument(
        "--pred_vcf",
        nargs="+",
        required=False,
        dest="pred_vcf",
    ...
    required_opts_group.add_argument(
        ...
        type=str,
        required=False,
        help="directory containing all vcfs to benchmark",
    )

    required_opts_group.add_argument(
        "--truth_vcf", required=True, help="reference truth set vcf"
    )
    required_opts_group.add_argument(
        "--pred_bam",
        required=True,
        dest="bam_file",
        help="Alignment file of reads with the reference",
    )

    ## general optional args
    optional_opts_group = arg_parser.add_argument_group("optional arguments")
    optional_opts_group.add_argument(
        "--output_dir",
        required=False,
        dest="output_dir",
        help="Output directory, default is set to pred_vcf and filtering options applied",
    )
    optional_opts_group.add_argument(
        "--cpu",
        required=False,
        dest="cpu",
        default=1,
        help="CPU count to be applied for multiprocessing.",
    )

    ## exome mode opts
    exome_opts_group = arg_parser.add_argument_group("exome reference mode")
    exome_opts_group.add_argument(
        "--exome_bam", help="Alignment file of the exome with the reference"
    )
    exome_opts_group.add_argument(
        "--min_exome_depth", type=int, help="min exome depth required", default=10
    )

    ## restrict to high conf region opts
    restrict_regions_opts_group = arg_parser.add_argument_group(
        "restrict evaluation regions on genome"
    )
    restrict_regions_opts_group.add_argument(
        "--restrict_regions_bed",
        help="bed file containing regions to restrict analysis to. (ie. high confidence regions)",
    )

    ## var annotation options used later for filtering or analysis.
    var_annotation_opts_group = arg_parser.add_argument_group(
        "variant annotations to include in variants table"
    )
    var_annotation_opts_group.add_argument(
        "--rna_editing", dest="rna_editing", help="input rna-editing vcf file"
    )

    ## variant filtering options:
    var_filtering_opts_group = arg_parser.add_argument_group(
        "variant filtering parameters"
    )
    var_filtering_opts_group.add_argument(
        "--min_RNAseq_Depth",
        type=int,
        help="Depth value for SNP analysis \n",
        default=1,
    )
    var_filtering_opts_group.add_argument(
        "--indels_only", action="store_true", default=False, help="indels_only"
    )
    var_filtering_opts_group.add_argument(
        "--snvs_only", action="store_true", default=False, help="snvs_only"
    )
    ...
    pred_vcfs = None
    if args.pred_vcf is not None:
        pred_vcfs = args.pred_vcf
    else:
        pred_vcf_dir = args.pred_vcf_dir
        if pred_vcf_dir is None:
            raise RuntimeError("Error, must specifiy --pred_vcf or --pred_vcf_dir ")

        pred_vcfs = list()
        pred_vcf_filenames = glob.glob(os.path.abspath(pred_vcf_dir) + "/*vcf.gz")
        logger.info("pred vcfs: {}".format(str(pred_vcf_filenames)))
        for pred_vcf in pred_vcf_filenames:
            pred_vcfs.append(pred_vcf)

    logger.info("-benchmarking vcfs: {}".format(str(pred_vcfs)))
    ...
    truth_vcf = os.path.abspath(args.truth_vcf)
    ...
    for pred_vcf in pred_vcfs:
        ...
        logger.info("\n\n********* -PROCESSING {}".format(pred_vcf))

        pred_vcf_basename = os.path.basename(pred_vcf)

        aggregated_data_table_filename = os.path.join(
            output_dir, pred_vcf_basename + ".aggregated.dat"
        )

        cmdstr = " ".join(
            [
                os.path.join(UTILDIR, "aggregate_variants_n_depth_to_table.py"),
                "--pred_vcf {}".format(pred_vcf),
                "--truth_vcf {}".format(truth_vcf),
                "--pred_bam {}".format(bam_file),
        ...
        if args.rna_editing:
            rna_editing_filename = os.path.abspath(args.rna_editing)
            cmdstr += " --rna_editing {} ".format(rna_editing_filename)

        pipeliner.add_commands(
            [Command(cmdstr, "aggregated_{}.ok".format(pred_vcf_basename))]
        )
        ...
        cmdstr = " ".join(
            [
                os.path.join(UTILDIR, "filterVariantTableByAttributes.py"),
        ...
        pipeliner.add_commands(
            [Command(cmdstr, "filtered_table_{}.ok".format(pred_vcf_basename))]
        )
        ...
        cmdstr = " ".join(
            [
                os.path.join(UTILDIR, "classify_TP_FP_FN.py"),
        ...
        pipeliner.add_commands(
            [Command(cmdstr, "scored_variants_{}.ok".format(pred_vcf_basename))]
        )

        # tracking some output files for summaries below.
        score_files.append(scored_variants_file)
        accuracy_stats_file = (
            scored_variants_file + ".accuracy_stats"
        )  ## created by EvaluateAccuracy.py below for each input file.
    ...
    cmdstr = " ".join(
        [
            os.path.join(UTILDIR, "EvaluateAccuracy.py"),
            "--input_tables {}".format(" ".join(score_files)),
            "--output_dir {}".format(output_dir),
        ]
    )
    pipeliner.add_commands(
        [Command(cmdstr, "eval_accuracy_stats_{}.ok".format(time.time()))]
    )
    ...
    cmdstr = " ".join(
        [
            os.path.join(UTILDIR, "Accuracy_statistics.py"),
            "--input_accuracy_table {}".format(" ".join(accuracy_files)),
            "--output_dir {}".format(output_dir),
        ]
    )
    pipeliner.add_commands(
        [Command(cmdstr, "Accuracy_statistics_plots.{}.ok".format(time.time()))]
    )
    ...
    cmdstr = " ".join(
        [
            os.path.join(UTILDIR, "plot_PR_n_ROC.py"),
            "--input_accuracy_table {}".format(" ".join(accuracy_files)),
            "--output_dir {}".format(output_dir),
        ]
    )
    ...
    if not args.indels_only:
        cmdstr = " ".join(
            [
                os.path.join(UTILDIR, "plot_SNP_freqs_barplot.py"),
        ...
        pipeliner.add_commands(
            [Command(cmdstr, "snp_freq_barplots.{}.ok".format(time.time()))]
        )
